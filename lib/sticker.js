...
const { catbox, top4top } = require('./uploadImage.js')
...
const tmp = path.join(__dirname, '../sampah')

/** ✅ pindahkan support ke atas */
const support = {
  ffmpeg: true,
  ffprobe: true,
  ffmpegWebp: true,
...
function sticker2(img, url) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let bufs = []
      /** ✅ gunakan support langsung, bukan module.exports */
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    const tmpfile = path.join(__dirname, `../sampah/${+ new Date()}.${type.ext}`)
